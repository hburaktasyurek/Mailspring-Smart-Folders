...
const HIDDEN_ROLES = new Set(['spam', 'trash']);
...
function categoriesByVisibility(categories) {
  ...
  categories.forEach((category) => {
    if (HIDDEN_ROLES.has(category.role)) {
...
function resolutionKeyFor(resolution) {
  return JSON.stringify({
    valid: resolution.validCategories.map((category) => [category.accountId, category.id]),
...
class SmartFolderPerspective extends MailboxPerspective {
  ...
  hasMixedHiddenCategories() {
    const {hidden, normal} = categoriesByVisibility(this.categories());
    return hidden.length > 0 && normal.length > 0;
  }
